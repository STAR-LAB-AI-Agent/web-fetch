...
页面结构由程序自动判断，顺序为 **表格 → 重复卡片/条目 → 详情页**；也可以从自然语言里识别：
出现「详情 / 这条 / 单条」等说法，或目标路径是 `/notices/N-01` 这类两段式路径时，按详情页处理。

## 参数与调用方式

- 入口：`python -m web_agent.cli agent --base <站点URL> --text "<自然语言>" [--out <文件>] [--kind auto|table|list|item]`
...
- 常用可选参数：`--pages 1,2|all`（多页）、`--dedup-by <字段>`（去重）、`--kind auto|table|list|item`、
  `--row-selector <选择器>`、`--headed`（显示浏览器）、`--force`（覆盖已有输出文件，默认会先询问）。
...
1. 解析自然语言 → 得到采集计划：目标路径、字段列表、页数、导出格式、是否去重、页面结构。
2. 打开目标页面确认结构：按 表格 → 重复卡片/条目 → 详情页 的顺序自动判断，两者都不像时按详情页解析。
3. 按字段抽取为行记录；页面中不存在的字段置空并在 `missing_fields` 中列出。
4. 多页采集时按指定字段去重，保留首次出现的记录。
5. 写出前检查目标文件是否已存在，存在则先询问用户是否覆盖，确认后才写入。
6. 按指定格式写出文件，并返回条数、重复条数与输出路径；一条都没采到时给出 `hint` 排查提示。
...
- 目标文件已存在时先询问用户是否覆盖，确认后才写入；非交互环境默认不覆盖。