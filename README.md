...
# 详情页：话里出现「这条 / 详情 / 单条」或路径是两段式时会自动按单条记录处理
python -m web_agent.cli agent --base http://127.0.0.1:5099 \
  --text "采集 /notices/N-01 这条公告的正文和附件链接" --out detail.json
```
...
`kind` 没有显式指定时，程序按 **表格 → 重复卡片/条目 → 详情页** 的顺序自动判断目标页面结构；也可以从自然语言里识别：出现「详情 / 这条 / 单条」等说法，或目标路径是 `/notices/N-01` 这类两段式路径时，直接按详情页处理。命令行可用 `--kind table|list|item` 强制覆盖自动判断。

字段名支持常见同义说法，例如「价格 → 参考价」「日期 → 发布时间」「详情/地址/网址 → 链接」；请求了页面上不存在的字段时，结果中该字段为空并在 `missing_fields` 中列出，不会静默丢失。如果一条记录都没采到（例如页面 404 或没有数据），返回结果里会带 `hint` 字段说明排查方向，不会只给一个空列表。
